		...
		value: function sync() {
			...
			_db2.default.table('convites').toArray().then(function (convites) {
				...
				retorno = retorno.map(function (element) {
					...
					if (element.sync == 1) element.sync = true;else element.sync = false;
					element.id = parseInt(element.id, 10);
					_db2.default.table('convites').where('id').equals(element.id).delete();
					_db2.default.table('convites').add(element);
				});
				_this3.refresh();
			});
		}
